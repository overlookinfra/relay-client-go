...
func (dst *RevisionSource) UnmarshalJSON(data []byte) error {
	...
	// check if the discriminator value is 'RevisionRelaySource'
	if jsonDict["source_type"] == "RevisionRelaySource" {
		// try to unmarshal JSON data into RevisionRelaySource
		err = json.Unmarshal(data, &dst.RevisionRelaySource)
		if err == nil {
			return nil // data stored in dst.RevisionRelaySource, return on the first match
		} else {
			dst.RevisionRelaySource = nil
			return fmt.Errorf("Failed to unmarshal RevisionSource as RevisionRelaySource: %s", err.Error())
		}
	}

	// check if the discriminator value is 'RevisionRepositorySource'
	if jsonDict["source_type"] == "RevisionRepositorySource" {
		// try to unmarshal JSON data into RevisionRepositorySource
		err = json.Unmarshal(data, &dst.RevisionRepositorySource)
		if err == nil {
			return nil // data stored in dst.RevisionRepositorySource, return on the first match
		} else {
			dst.RevisionRepositorySource = nil
			return fmt.Errorf("Failed to unmarshal RevisionSource as RevisionRepositorySource: %s", err.Error())
		}
	}

	// check if the discriminator value is 'relay'
	if jsonDict["source_type"] == "relay" {
		// try to unmarshal JSON data into RevisionRelaySource
		err = json.Unmarshal(data, &dst.RevisionRelaySource)
		if err == nil {
			return nil // data stored in dst.RevisionRelaySource, return on the first match
	...
	if jsonDict["source_type"] == "repository" {
		// try to unmarshal JSON data into RevisionRepositorySource
		err = json.Unmarshal(data, &dst.RevisionRepositorySource)
		if err == nil {
			return nil // data stored in dst.RevisionRepositorySource, return on the first match
